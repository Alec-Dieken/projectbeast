import os, secrets, random, tinify, requests
...
from projectbeast.forms import (
    SignupForm,
    LoginForm,
    RequestResetForm,
    ResetPasswordForm,
    ...
    AddTaskForm,
    AddGroupForm,
    AddProjectFormNoGroup,
    AddGroupMemberForm,
    EditAccountForm,
    UploadImage,
    EditGroupForm,
    AddTaskFormNoProject,
)
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            if user.is_social_account():
                flash(
                    "Please login using the social media account you registered with.",
                    "error",
                )
                return redirect(url_for("login"))
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        if user is None or user.email == "test@demouser.com":
            flash(
                "If that address is associated with an account, please check your email for further instructions.",
                "info",
            )
            return redirect(url_for("login"))
    ...
    return render_template(
        "/auth/reset-password.html", pb_title="Reset Password", form=form
    )
...
def reset_token(token):
    ...
    return render_template(
        "/auth/reset-token.html", pb_title="Reset Password", form=form
    )
...
def facebook_auth():
    ...
    try:
        ...
        user_data = facebook.get(
            "https://graph.facebook.com/me?fields=first_name,last_name,email,picture.width(256).height(256){url}"
        ).json()
        ...
        try:
            ...
            if response.status_code != 200:
                profile_pic = "/static/images/global/default1.svg"
        except:
            profile_pic = "/static/images/global/default1.svg"
        ...
        if check_user:
            ...
            password_hash = bcrypt.generate_password_hash(secrets.token_hex(24)).decode(
                "utf-8"
            )
...
def github_auth():
    ...
    if not email:
        flash("GitHub account must have a public email", "error")
        return redirect(url_for("login"))
    ...
    if check_user:
        ...
        password_hash = bcrypt.generate_password_hash(secrets.token_hex(16)).decode(
            "utf-8"
        )
...
def dashboard():
    ...
    add_task_form.project_id.choices = [
        (
            project["id"],
            f"{project['name']} ({project['group'].group_name if project['group'] else 'none'})",
        )
    ...
    priority_chart_data = {
        ...
        "med": len(med_priority),
        "high": len(high_priority),
        "imm": len(imm_priority),
    }
    type_chart_data = {
        "bug": len(type_bug),
        "feature": len(type_feature),
        "other": len(type_other),
    }

    type_is_empty = (
        True if len(type_bug) + len(type_feature) + len(type_other) == 0 else False
    )
    ...
    return render_template(
        ...
        priority_is_empty=priority_is_empty,
...
def account(username):
    ...
    if image_form.submit.data and image_form.validate_on_submit():
        ...
        try:
            path = os.path.join(
                "/home/ajdieken/projectbeast/projectbeast/",
                "static/images/users",
                filename,
            )
    ...
    if user.is_demo_account:
        ...
        for request in sent_requests:
            req_rec = RequestReceivers.query.filter(
                RequestReceivers.request_id == request.id
            ).all()
            for r in req_rec:
                db.session.delete(r)
                db.session.commit()
...
def search_results():
    ...
    active_peer_requests = Requests.query.filter(
        and_(
            Requests.sender_id == current_user.id,
            Requests.type == "add-peer",
            or_(Requests.is_active == True, Requests.result == "Declined"),
        )
    ).all()
    ...
    for req in active_peer_requests:
        req_rec = RequestReceivers.query.filter(
            RequestReceivers.request_id == req.id
        ).first()
    ...
    return render_template(
        ...
        user=current_user,
        add_group_form=add_group_form,
        peer_request_ids=peer_request_ids,
    )
...
def peers_page():
    ...
    return render_template(
        "/dashboard/peers.html", user=user, peers=peers, add_group_form=add_group_form
    )
...
def peer_request(peer_id):
    ...
    current_user_requests = Requests.query.filter(
        and_(
            Requests.type == "add-peer",
            Requests.sender_id == current_user.id,
    ...
    for request in current_user_requests:
        if RequestReceivers.query.filter(
            and_(
                RequestReceivers.request_id == request.id,
                RequestReceivers.receiver_id == peer_id,
            )
        ).first():
            return redirect(session["previous_url"])
    ...
    new_req_rec = RequestReceivers(request_id=new_request.id, receiver_id=peer_id)
...
def add_peer(req_id):
    ...
    if not RequestReceivers.query.filter(
        and_(
            RequestReceivers.receiver_id == current_user.id,
            RequestReceivers.request_id == req_id,
        )
    ).first():
        return redirect(session["previous_url"])
...
def create_group():
    ...
    if add_group_form.validate_on_submit():
        ...
        new_group = Groups(
            group_name=group_name, group_image=group_image, group_link=group_link
        )
        db.session.add(new_group)
        db.session.flush()
        group_ra = RecentActivity(
            user_id=current_user.id,
            type="new_group",
        ...
        group_rel = UserGroups(
            user_id=current_user.id, group_id=new_group.id, role="Creator"
        )
...
def group_page(group_link):
    ...
    if group is None or not current_user.is_in_group(group.id):
        ...
        is_admin = (
            ...
            if UserGroups.query.filter(
                and_(
                    UserGroups.group_id == group.id,
                    UserGroups.user_id == current_user.id,
                    or_(UserGroups.role == "Admin", UserGroups.role == "Creator"),
                )
            ).first()
            else False
        )
        ...
        if UserGroups.query.filter(
            and_(UserGroups.group_id == group.id, UserGroups.role == "Creator")
        ).first():
            group_creator = Users.query.get(
                UserGroups.query.filter(
                    and_(UserGroups.group_id == group.id, UserGroups.role == "Creator")
                )
                .first()
                .user_id
        ...
        group_admins = [
            group_user
            for group_user in members
            if UserGroups.query.filter(
                and_(
                    UserGroups.group_id == group.id,
                    UserGroups.user_id == group_user.id,
                    UserGroups.role == "Admin",
                )
            ).first()
        ]
        ...
        project_choices = Projects.query.filter(
            and_(Projects.group_id == group.id, Projects.is_active == True)
        ).all()
        add_task_form.project_id.choices = [
        ...
        add_group_member_form.peers.choices = [
            (user.id, user.username)
            for user in peers
            if not UserGroups.query.filter(
                and_(UserGroups.group_id == group.id, UserGroups.user_id == user.id)
            ).first()
        ]
        ...
        return render_template(
            ...
            add_task_form=add_task_form,
            user=current_user,
            active_projects=group_active_projects,
            active_tasks=group_active_tasks,
            group_creator=group_creator,
            group_admins=group_admins,
            is_admin=is_admin,
            add_group_member_form=add_group_member_form,
...
def edit_group(group_link):
    ...
    user_group = UserGroups.query.filter(
        and_(UserGroups.user_id == current_user.id, UserGroups.group_id == group.id)
    ).first()
...
def delete_group(group_link):
    ...
    for project in group_projects:
        ...
        active_tasks = Tasks.query.filter(
            and_(Tasks.project_id == project.id, Tasks.is_active == True)
        ).all()
    ...
    if len(active_group_tasks) > 0:
        ...
        group_users = UserGroups.query.filter(UserGroups.group_id == group.id).all()
        for task in all_group_tasks:
            user_tasks = UserTasks.query.filter(UserTasks.task_id == task.id).all()
        ...
        for project in group_projects:
            user_projects = UserProjects.query.filter(
                UserProjects.proj_id == project.id
            ).all()
    ...
    for project in projects:
        ...
        for task in tasks:
            user_task = UserTasks.query.filter(
                and_(UserTasks.user_id == member_id, UserTasks.task_id == task.id)
            ).first()
            db.session.delete(user_task)
        user_proj = UserProjects.query.filter(
            and_(UserProjects.user_id == member_id, UserProjects.proj_id == project.id)
        ).first()
...
def promote_group_member(group_link, member_id):
    ...
    member_user_group = UserGroups.query.filter(
        and_(UserGroups.group_id == group.id, UserGroups.user_id == member.id)
    ).first()
    my_user_group = UserGroups.query.filter(
        and_(UserGroups.group_id == group.id, UserGroups.user_id == current_user.id)
    ).first()
    ...
    if (
        group is None
        or member is None
        or member_user_group is None
        or role == "Admin"
        or role == "Creator"
        or my_user_group is None
        or my_user_group.role == "Member"
    ):
        return redirect(session["previous_url"])
    else:
...
def remove_group_member(group_link, member_id):
    ...
    if (
        ...
        return redirect(session["previous_url"])
    elif (my_user_group.role == "Admin" and role == "Member") or (
        my_user_group.role == "Creator" and (role == "Member" or role == "Admin")
    ):
        projects = Projects.query.filter(Projects.group_id == group.id).all()
        for project in projects:
            tasks = Tasks.query.filter(Tasks.project_id == project.id).all()
            ...
            user_proj = UserProjects.query.filter(
                and_(
                    UserProjects.user_id == member_id,
                    UserProjects.proj_id == project.id,
                )
            ).first()
            db.session.delete(user_proj)
    ...
    if add_group_member_form.validate_on_submit():
        ...
        for user_id in users:
            ...
            if request_exists is False:
                ...
                db.session.add(new_request)
                db.session.flush()
                new_request.action_url = (
                    f"/group/{group_link}/members/add/{new_request.id}"
                )
...
def join_group(group_link, req_id):
    ...
    request = Requests.query.filter(
        and_(Requests.id == req_id, Requests.is_active == True)
    ).first()
    req_rec = RequestReceivers.query.filter(
        and_(
            RequestReceivers.request_id == req_id,
    ...
    user_group_exists = UserGroups.query.filter(
        and_(UserGroups.group_id == group.id, UserGroups.user_id == current_user.id)
    ).first()
    ...
    request.is_active = False
    request.result = "Accepted"
    user_group = UserGroups(user_id=current_user.id, group_id=group.id, role="Member")
    ...
    for project in new_projects:
        new_user_proj = UserProjects(user_id=current_user.id, proj_id=project.id)
...
def create_task(group_link):
    ...
    add_task_form.priority.choices = ["Low", "Medium", "High", "Immediate"]
    add_task_form.type.choices = ["Bug", "Feature", "Other"]
    project_choices = Projects.query.filter(Projects.group_id == group.id).all()
    add_task_form.project_id.choices = [
        (project.id, project.project_name) for project in project_choices
    ]
...
def project_view(group_link, project_id):
    ...
    active_tasks = [
        task
        for task in current_user.get_active_tasks(group.id)
...
# ##################################################################################################################
@app.route(
    "/group/<string:group_link>/<int:project_id>/<int:task_id>/complete",
    methods=["POST"],
)
@login_required
...
# ##################################################################################################################
@app.route(
    "/group/<string:group_link>/<int:project_id>/<int:task_id>/delete", methods=["POST"]
)
@login_required
...
def messages_home():
    ...
    info_for_message_link = Messages.get_message_link_info(rooms, current_user.id)

    return render_template(
        "/dashboard/messages.html",
        info_for_message_link=info_for_message_link,
        user=current_user,
    )
...
def private_messaging(peer_un):
    ...
    add_group_form = AddGroupForm()
    messages = (
        Messages.query.filter(Messages.room_name == peer_obj.id)
